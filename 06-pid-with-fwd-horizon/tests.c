...
int test_pid ( double duration, double kp, double ki, double kd, char* out_name ) {
  ...
  // TODO: insetad of cputime, should use PRU time.
  ...
  for( int i=0; i<num_iters; i++ ) {
    ...
    get_data_cmd_bufs( &(db_buffer[i]), &(cb_rx[i]) ); // write it to the huge history of buffers for writing the data file later,
    ...
    angle[i]   	  =     eqep_to_angle( db_buffer[i][PKTS_PER_DATA_BUFFER-1].eqep );
    ...
    double cur_int2 = error2 * dt;
    double cur_int3 = error3 * dt;
