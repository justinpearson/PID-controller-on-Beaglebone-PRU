...
int main ( int argc, char *argv[] ) {
  ...
  toc(&(cputime[0]), &(cputimediff[0]) ); // just to initialize cputime around the same time as the PRU.
  ...
  for( int i=0; i<num_iters; i++ ) {
    ...
    double prev_angle = i>0 ? angle[i-1] : 0;
#endif
    ...
    //IMPORTANT: We should "catch up" the integral term, but I haven't coded that yet.
    ...
    double slope  = i>0 ? (angle[i]-prev_angle)/dt : 0;
    double angle2 = angle[i] + slope * DT;
    double angle3 = angle2   + slope * DT;
    double angle4 = angle3   + slope * DT;
    double angle5 = angle4   + slope * DT;

#if REFTYPE == 0 // sine wave
    ref[i]     	  = 180 * sin(2.0 * M_PI * freq * t); // deg
    double ref2   = 180 * sin(2.0 * M_PI * freq * (t+DT)); // deg
    double ref3   = 180 * sin(2.0 * M_PI * freq * (t+DT+DT)); // deg
    ...
    //    ref[i] =     4*a/p*(fabs(fmod(t-p/4,p)-p/2) - p/4);
    ...
    ref[i] =        4*a/p*(fabs(  (t-p/4)-p*floor((t-p/4)/p)  -p/2) - p/4);
    double ref2 =   4*a/p*(fabs(  (t+DT-p/4)-p*floor((t+DT-p/4)/p)  -p/2) - p/4);
    double ref3 =   4*a/p*(fabs(  (t+DT+DT-p/4)-p*floor((t+DT+DT-p/4)/p)  -p/2) - p/4);
    double ref4 =   4*a/p*(fabs(  (t+DT+DT+DT-p/4)-p*floor((t+DT+DT+DT-p/4)/p)  -p/2) - p/4);
    double ref5 =   4*a/p*(fabs(  (t+DT+DT+DT+DT-p/4)-p*floor((t+DT+DT+DT+DT-p/4)/p)  -p/2) - p/4);
#endif

    error[i]   	  = ref[i]-angle[i];
    double error2        = ref2-angle2;
    double error3        = ref3-angle3;
    double error4        = ref4-angle4;
    double error5        = ref5-angle5;

    // Integral term: 
    // - if 1st iter, don't add prev integral
    // - if real delta-time <=0, don't add current integral.
    double prev_int = i>0 ? ierror[i-1] : 0;
    double cur_int  = dt>0 ? error[i]*dt : 0;
    ...
    double cur_int4 = error4 * DT; // don't know what the future DT will be (w/ preempt), but guess.
    double cur_int5 = error5 * DT; // don't know what the future DT will be (w/ preempt), but guess.

    double ierror2  = prev_int + cur_int + cur_int2;
    double ierror3  = prev_int + cur_int + cur_int2 + cur_int3;
    double ierror4  = prev_int + cur_int + cur_int2 + cur_int3 + cur_int4; 
    double ierror5  = prev_int + cur_int + cur_int2 + cur_int3 + cur_int4 + cur_int5;

    derror[i]       = dt>0 && i>0 ? (error[i] - error[i-1])/dt : 0;
    double derror2  = (error2 - error[i])/DT;
    double derror3  = (error3 - error2)/DT;
    double derror4  = (error4 - error3)/DT;
    double derror5  = (error5 - error4)/DT;

    pterm[i]        = kp * error[i];
    double pterm2   = kp * error2;
    double pterm3   = kp * error3;
    double pterm4   = kp * error4;
    double pterm5   = kp * error5;

    iterm[i]        = ki * ierror[i];
    double iterm2   = ki * ierror2;
    double iterm3   = ki * ierror3;
    double iterm4   = ki * ierror4;
    double iterm5   = ki * ierror5;
    ...
    double dterm4 = kd * derror4;
    double dterm5 = kd * derror5;

    v[i]          = pterm[i] + iterm[i] + dterm[i];
    double v2     = pterm2 + iterm2 + dterm2;
    double v3     = pterm3 + iterm3 + dterm3;
    double v4     = pterm4 + iterm4 + dterm4;
    double v5     = pterm5 + iterm5 + dterm5;
    ...
    if( v3<-MAX_VOLTAGE) v3=-MAX_VOLTAGE;
    if( v4>MAX_VOLTAGE ) v4=MAX_VOLTAGE;
    if( v4<-MAX_VOLTAGE) v4=-MAX_VOLTAGE;
    if( v5>MAX_VOLTAGE ) v5=MAX_VOLTAGE;
    if( v5<-MAX_VOLTAGE) v5=-MAX_VOLTAGE;
