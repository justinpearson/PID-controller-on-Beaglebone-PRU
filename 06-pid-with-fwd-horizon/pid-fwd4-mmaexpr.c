...
not done with this guy, need to make sure it works on the sysfs version first.
...
inline double pid_iir( double ek, double ekm1, double ekm2, double vkm1, double vkm2 ) {      
...
int main ( int argc, char *argv[] ) {
  ...
  /* const char* out_name      = argc>4 ? argv[4]  : "pid-fwd.txt";    // output file name */
...
#define flog_buf_size  999999
 ...
 pid_iir( double ek, double ekm1, double ekm2, double vkm1, double vkm2 );
    ...
    if( sn==snprev ) {
      ...
      double prev_prev_angle = 180.0 / M_PI *  eqep_to_angle( db_buffer[i][PKTS_PER_DATA_BUFFER-3].eqep );
      refs[i] = reference(DT*sn);
      double error      = refs[i]- angle;
      double prev_error = reference(DT*(sn-1)) - prev_angle;
      double prev_prev_error = reference(DT*(sn-2)) - prev_prev_angle;
      voltages[i] = gpio_duty_to_motor_voltage_cmd( 
						   db_buffer[i][PKTS_PER_DATA_BUFFER-1].gpio,
						   db_buffer[i][PKTS_PER_DATA_BUFFER-1].duty
						    );

      volt = gpio_duty_to_motor_voltage_cmd( 
					    db_buffer[i][PKTS_PER_DATA_BUFFER-1].gpio,
					    db_buffer[i][PKTS_PER_DATA_BUFFER-1].duty
					     );
      prev_volt = gpio_duty_to_motor_voltage_cmd( 
						 db_buffer[i][PKTS_PER_DATA_BUFFER-2].gpio,
						 db_buffer[i][PKTS_PER_DATA_BUFFER-2].duty
						  );
