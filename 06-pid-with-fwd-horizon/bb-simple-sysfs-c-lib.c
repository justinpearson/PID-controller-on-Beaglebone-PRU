...
int fd_gpio_ain1_direction, fd_gpio_ain1_value; // GPIO pin for motor driver pin "AIN1"
int fd_gpio_ain2_direction, fd_gpio_ain2_value; // GPIO pin for motor driver pin "AIN1"
int fd_gpio_stby_direction, fd_gpio_stby_value; // GPIO pin for motor driver pin "standby"
...
void setup() {
  ...
  fd_gpio_ain1_direction = open(GPIO_AIN1_PATH "direction", w);
  fd_gpio_ain1_value     = open(GPIO_AIN1_PATH "value",     w);
  fd_gpio_ain2_direction = open(GPIO_AIN2_PATH "direction", w);
  fd_gpio_ain2_value     = open(GPIO_AIN2_PATH "value",     w);
  ...
  if( fd_gpio_ain1_direction == -1 || fd_gpio_ain1_value == -1 ) {
    printf("Couldn't find " GPIO_AIN1_PATH ", prob need to \n$ echo 111 > /sys/class/gpio/export\n");
    exit(EXIT_FAILURE);
  }
  if( fd_gpio_ain2_direction == -1 || fd_gpio_ain2_value == -1 ) {
    printf("Couldn't find " GPIO_AIN2_PATH ", prob need to \n$ echo 112 > /sys/class/gpio/export\n");
    exit(EXIT_FAILURE);
  }
  ...
  // GPIO direction
  write( fd_gpio_ain1_direction, "out", 3 );
  write( fd_gpio_ain2_direction, "out", 3 );
  ...
  write(fd_eqep_period, "10000000",9);  // ns => 0.01 sec
  //  printf("FYI, setting the EQEP period to 10000000 ns = 0.01sec, can change in file %s function %s line %d\n",__FILE__,__FUNCTION__,__LINE__);
...
void shutdown() {
  ...
  close(fd_pwm_period);
  close(fd_pwm_run);
  close(fd_gpio_ain1_direction);
  close(fd_gpio_ain1_value);
  close(fd_gpio_ain2_direction);
  close(fd_gpio_ain2_value);
...
// GPIO
void stby()   { check_setup(); write(fd_gpio_stby_value,"0",1); }
void unstby() { check_setup(); write(fd_gpio_stby_value,"1",1); }
void cw()     { check_setup(); write(fd_gpio_ain1_value, "1",1); write(fd_gpio_ain2_value, "0",1); }
void ccw()    { check_setup(); write(fd_gpio_ain1_value, "0",1); write(fd_gpio_ain2_value, "1",1); }
...
int eqep_counts() {
  ...
  if( i==0 ) { 
    printf("Nothing read! Uh oh.\n"); 
    printf("pread returned: %d (%s)\n",i, strerror(errno));
    exit(EXIT_FAILURE);
  }
  else if( i<0 ) {
    printf("Some pread failure!\n");
    printf("pread returned: %d (%s)\n",i, strerror(errno));
    exit(EXIT_FAILURE);
  }
  int enc = 999;
