...
//////////////////////////////////////                                                                     
// From Joao's tenscalc server code.                                                                       
...
int main ( int argc, char *argv[] ) {
  ...
  // Start listening for a command buffer.
...
#define flog_buf_size  999999
  ...
  for( int i=0; i<MAX_ITERS; i++ ) {
    ...
    vkm1 = -voltages_pru[sn]; // pru wired backwards; minus sign needed.
    vkm2 = -voltages_pru[sn-1];
    ...
    for( int j=sn+1; j<sn+1+T; j++ ) {
      voltages_pru[j] = -v; // PRU wiring error: minus sign needed.
    }
#else
    for( int j=sn+1; j<sn+1+T; j++ ) {
      vk = pid_iir( ek, ekm1, ekm2, vkm1, vkm2 );
      vk = CLAMP(vk,MAX_VOLTAGE); 
      voltages_pru[j] = -vk; // PRU wiring error: minus sign needed.
      ...
      angles_pru[j] = sys_iir(  angles_pru[j-1],
				angles_pru[j-2],
				angles_pru[j-3],
				-voltages_pru[j-0], // need minus signs here? Also: j or j-1??
				-voltages_pru[j-1],
				-voltages_pru[j-2]);
